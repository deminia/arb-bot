...
_extra_env = _s("EXTRA_BOOKMAKERS", "")
_extra_auto: list[str] = []
if CLOUDBET_API_KEY: _extra_auto.append("cloudbet")
if STAKE_API_KEY:    _extra_auto.append("stake")
_extra_set = {b.strip().lower() for b in _extra_env.split(",") if b.strip()}
...
async def refetch_valuebet_odds(vb: "ValueBetSignal") -> tuple[float, bool]:
    ...
    try:
        ...
        # C1 fix: use extra feed for any bm NOT in main BOOKMAKERS list
        # (no longer requires bm to be explicitly listed in EXTRA_BOOKMAKERS env)
...
async def cmd_debug(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    extra_bm  = EXTRA_BOOKMAKERS_EFFECTIVE
    ...
    lines.append(f"Extra : `{extra_bm if extra_bm else 'ไม่ได้ตั้ง EXTRA_BOOKMAKERS / API keys'}`")
    lines.append(f"")
